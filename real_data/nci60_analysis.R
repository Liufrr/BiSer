################################################################################
# NCI60 Dataset Analysis — Unsupervised Feature Selection (Variance)
#
# Microarray data (ISLR::NCI60), 4 cancer types subset:
#   MELANOMA, LEUKEMIA, NSCLC, COLON
# Uses unsupervised variance ranking for gene selection, then runs all methods.
#
# Produces:
#   - Heatmap comparisons (2x4 grid with group annotation bar)
#   - ARI vs NMI scatter plot
#   - Combined Figure_NCI60.png
################################################################################
...
nci_colors <- c("LEUKEMIA" = "#E64B35FF", "MELANOMA" = "#4DBBD5FF",
                "NSCLC"    = "#00A087FF", "COLON"    = "#3C5488FF")
...
# ============================================================
# 2. Load Data & Variance-based Feature Selection
# ============================================================

data(NCI60)

# Subset to 4 cancer types
target_types <- c("MELANOMA", "LEUKEMIA", "NSCLC", "COLON")
keep_idx     <- which(NCI60$labs %in% target_types)
raw_matrix   <- t(NCI60$data[keep_idx, ])          # genes x samples
group_labels <- droplevels(factor(NCI60$labs[keep_idx]))

cat("Raw matrix:", nrow(raw_matrix), "genes x", ncol(raw_matrix), "samples\n")
cat("Group labels:", paste(levels(group_labels), collapse = ", "), "\n")
cat("Sample counts per group:\n")
print(table(group_labels))

# Unsupervised feature selection (variance ranking, standard for microarray)
n_top <- 20
feat  <- get_clustering_matrix(raw_matrix, n_top = n_top, method = "variance", norm = FALSE)
...
mat0 <- data_scaled
cat("After feature selection:", nrow(mat0), "genes x", ncol(mat0), "samples\n")
...
# BiSer
allout$biser <- biser(mat1, noise = T, pct = 0.3, simmeth = "cor")
...
# Derive sample labels for mat1 columns
...
cat("mat1 dimensions:", m, "genes x", p, "samples\n")
...
# Group legend
group_legend <- Legend(
  labels    = names(nci_colors),
  legend_gp = gpar(fill = nci_colors),
  title     = "Group",
  title_gp  = gpar(fontsize = 26, fontface = "bold", fontfamily = "sans"),
...
fig_nci <- plot_grid(panel_A, panel_B,
                     nrow = 1, rel_widths = c(0.60, 0.40))

ggsave("Figure_NCI60.png", fig_nci,
       width = 28, height = 12, dpi = 500)

save(allout, file = "allout_NCI60.RData")


# ---- Print summary ----
cat("\n========================================\n")
cat("NCI60 Dataset — Unsupervised Feature Selection Summary\n")
cat("========================================\n")
cat("Feature selection: Variance ranking (unsupervised), top", n_top, "\n")
cat("Cancer types:", paste(target_types, collapse = ", "), "\n")
cat("Matrix dimensions:", nrow(mat1), "genes x", ncol(mat1), "samples\n\n")
cat("--- Metrics ---\n")
print(metrics_clean[, c("Method", "ARI", "NMI")], row.names = FALSE)
cat("========================================\n")
cat("Figure saved to: Figure_NCI60.png\n")
