################################################################################
# Khan Dataset Analysis — Unsupervised Feature Selection (HVG)
#
# Microarray data (ISLR::Khan), 4 cancer subtypes: BL, EWS, NB, RMS
...
khan_colors <- c("BL"  = "#E64B35FF", "EWS" = "#4DBBD5FF",
                 "NB"  = "#00A087FF", "RMS" = "#3C5488FF")
...
raw_matrix    <- t(Khan$xtrain)        # genes x samples
sample_labels <- Khan$ytrain
rownames(raw_matrix) <- paste0("g", 1:nrow(raw_matrix))
colnames(raw_matrix) <- paste0("s", 1:ncol(raw_matrix))

# Map numeric labels to cancer subtypes
group_labels <- character(length(sample_labels))
group_labels[sample_labels == 1] <- "BL"
group_labels[sample_labels == 2] <- "EWS"
group_labels[sample_labels == 3] <- "NB"
group_labels[sample_labels == 4] <- "RMS"
group_labels <- as.factor(group_labels)

# Unsupervised feature selection (variance ranking, standard for microarray)
n_top <- 50
feat  <- get_clustering_matrix(raw_matrix, n_top = n_top, method = "variance", norm = F)
...
# BiSer
allout$biser <- biser(mat1, noise = F, pct = 0.6, simmeth = "cor")
...
# Group legend
group_legend <- Legend(
  labels    = names(khan_colors),
  legend_gp = gpar(fill = khan_colors),
  title     = "Group",
  title_gp  = gpar(fontsize = 26, fontface = "bold", fontfamily = "sans"),
...
fig_khan <- plot_grid(panel_A, panel_B,
                      nrow = 1, rel_widths = c(0.60, 0.40))

ggsave("Figure_Khan.png", fig_khan,
       width = 28, height = 12, dpi = 500)

# ---- Print summary ----
cat("\n========================================\n")
cat("Khan Dataset — Unsupervised Feature Selection Summary\n")
cat("========================================\n")
...
cat("========================================\n")
cat("Figure saved to: Figure_Khan.png\n")
